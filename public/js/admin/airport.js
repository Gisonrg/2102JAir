function populateTable() {
	...
	$.getJSON('/api/airport', function(data) {
		if (data.length == 0) {
			tableContent += '<tr><td colspan="5" style="text-align:center;"><i>Empty</i></td></tr>';
			$('#data-entry').append($(tableContent));
		} else {
			$.each(data, function() {
				tableContent += '<div id="a' + this.code + '" class="reveal-modal small" data-reveal style="text-align:center;">';
				tableContent += '<h2>Delete airport ' + this.code + '</h2>';
				tableContent += '<h4>Are you sure?</h4>';
				tableContent += '<p><button class="alert" id="delete_' + this.code + '">Delete</button>';
				...
				tableContent += '</td><td style="text-align:center;">';
				tableContent += '<a id="' + this.code + '"><i class="icon-remove icon-2x"></i></a></td></tr>';
				$('#data-entry').append(tableContent);
				tableContent = '';
